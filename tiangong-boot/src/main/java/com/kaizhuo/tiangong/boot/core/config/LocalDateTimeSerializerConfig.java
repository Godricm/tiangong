package com.kaizhuo.tiangong.boot.core.config;
 ...
 * @program: tiangong
 * @package: com.kaizhuo.tiangong.boot.core.config
 * @description: springboot默认使用jackson序列化，配置接收参数时使用的LocalXxx时间格式
 * 使用此配置后，controller中接收参数使用的对象中，LocalDateTime类型的字段可以直接接收yyyy-MM-dd HH:mm:ss格式字符串的参数了，不用单独加注解配置单个字段的时间序列化格式
 * @author: miaochen
